...
export const KobilLlmConfigView: React.FC = () => {
  ...
  // Admin AI Studio Image Generation Test State
  const SAMPLE_HOUSE_IMAGE = 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=1200&q=80';
  const [testOriginalUrl, setTestOriginalUrl] = useState<string>(SAMPLE_HOUSE_IMAGE);
  const [testPrompt, setTestPrompt] = useState<string>('Ubah foto rumah ini menjadi suasana twilight / mode malam hari dengan lampu teras menyala hangat');
  const [testEnhancedUrl, setTestEnhancedUrl] = useState<string | null>(null);
  ...
  const handleTestAdminImageGeneration = async () => {
    if (!testOriginalUrl) {
      toast.error('Foto Wajib Diisi', 'Silakan pilih foto sampel atau unggah foto untuk dites.');
      return;
    }
    if (!testPrompt.trim()) {
      toast.error('Prompt Wajib Diisi', 'Silakan ketik instruksi prompt untuk pengujian.');
      return;
    }
    ...
    try {
      ...
      const usageLog = {
        ...
        image_id: data?.image_id || `img-test-${Date.now()}`,
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      if (error || !data?.success) {
        replyText = `⚠️ ${data?.error || error?.message || 'Gagal memproses respons AI'}`;
        ...
        const usageLog = {
          ...
          cost_estimate_usd: 0,
          status: 'failed',
          error_code: data?.status ? `HTTP_${data.status}` : 'CHAT_ERROR',
          created_at: new Date().toISOString(),
        };
        ...
        replyText = data.reply || 'Berhasil terhubung.';
        ...
        const usageLog = {
          ...
          provider: chatProvider,
          model: chatModel,
          tokens_used: data.usage?.total_tokens || Math.ceil(msgText.length / 4) + 60,
          latency_ms: latencyMs,
      ...
      const latencyMs = Date.now() - startTime;
      replyText = `⚠️ Gagal menghubungkan ke proxy server chat: ${err.message || 'Network error'}`;
    }
  ...
  return (
    <div className="space-y-8 animate-in fade-in" data-testid="system-api-keys-view">
      ...
      {/* Interactive Admin AI Studio Test Panel */}
      <div className="p-6 md:p-8 rounded-3xl bg-slate-900/80 border border-purple-500/30 backdrop-blur-xl shadow-xl space-y-6">
        <div className="flex items-center justify-between pb-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-heading font-bold text-base text-white">Pengujian Realtime AI Studio (Image Generation)</h3>
              <p className="text-xs text-slate-400">Verifikasi langsung alur pengolahan foto AI Studio (Foto Asli + Prompt → AI Result)</p>
            </div>
          </div>
          <span className="text-[10px] font-mono px-2.5 py-1 rounded-lg bg-purple-950 border border-purple-500/40 text-purple-300 font-bold">
            purpose='image_generation'
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-slate-300 mb-1.5 block">1. Pilih Foto Sampel / Unggah Foto Tes</label>
              <div className="flex items-center gap-3">
                <input
                  type="text"
                  value={testOriginalUrl}
                  onChange={(e) => setTestOriginalUrl(e.target.value)}
                  className="flex-1 text-xs rounded-xl bg-slate-950 border border-white/10 px-3 py-2.5 text-white font-mono focus:outline-none focus:ring-2 focus:ring-purple-500"
                  placeholder="URL Foto Properti (misal: https://...)"
                />
                <label className="px-3 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs text-white font-semibold cursor-pointer border border-white/10 flex items-center gap-1.5 shrink-0">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (file) {
                        setTestOriginalUrl(URL.createObjectURL(file));
                      }
                    }}
                  />
                </label>
            ...
            <div>
              <label className="text-xs font-semibold text-slate-300 mb-1.5 block">2. Instruksi Prompt Pengujian</label>
              <textarea
                rows={3}
                value={testPrompt}
                onChange={(e) => setTestPrompt(e.target.value)}
                className="w-full text-xs rounded-xl bg-slate-950 border border-white/10 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 leading-relaxed"
                placeholder="Tulis instruksi pengubahan foto..."
              />
            </div>

            <button
              type="button"
              onClick={handleTestAdminImageGeneration}
              disabled={isTestingImage}
              className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-semibold text-xs shadow-lg shadow-purple-500/20 flex items-center justify-center gap-2 transition-all disabled:opacity-50"
            >
              {isTestingImage ? (
                <>
                  <RefreshCw className="w-4 h-4 animate-spin" />
                  <span>Memproses AI Image Generation...</span>
                </>
              ) : (
                <>
                  <Sparkles className="w-4 h-4" />
                  <span>⚡ Uji Generate Gambar AI Studio Realtime</span>
                </>
              )}
            </button>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-semibold text-slate-300">3. Hasil Slider Sebelum vs Sesudah AI</label>
              {imageTestLatency && (
                <span className="text-[11px] font-mono text-purple-300">
                  ⚡ Latensi: {imageTestLatency}ms
                </span>
              )}
            </div>
            ...
            {testEnhancedUrl ? (
              <BeforeAfterSlider
                ...
                enhancedUrl={testEnhancedUrl}
                aspectRatio="16/9"
                beforeLabel="SEBELUM"
                afterLabel="SESUDAH (AI)"
              />
            ) : (
              <div className="aspect-[16/9] rounded-2xl bg-slate-950 border border-dashed border-white/10 flex flex-col items-center justify-center p-6 text-center text-slate-500 space-y-2">
                <ImageIcon className="w-8 h-8 text-purple-500/40" />
                <p className="text-xs text-slate-400">Klik tombol <strong>"⚡ Uji Generate Gambar AI Studio Realtime"</strong> untuk melihat hasil perbandingan slider.</p>
              </div>
            )}
          </div>
        </div>
